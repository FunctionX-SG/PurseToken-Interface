...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    let totalTransferAmount = myJson["TransferTotal"][0]
    let sum30TransferAmount = myJson["Transfer30Days"][0]
    let totalBurnAmount = myJson["BurnTotal"][0]
    let sum30BurnAmount = myJson["Burn30Days"][0]
    ...
    if (this.state.wallet === false && this.state.walletConnect === false) {
      ...
      let purseTotalSupplyOnFXCore = await this.loadPurseTotalSupplyOnFXCore()
      this.setState({ purseTotalSupplyOnFXCore })

      let fxRewardPerWeek = await this.loadMasterChef()
      this.setState({ fxRewardPerWeek: fxRewardPerWeek.toString() })
    }
  }
  ...
  async loadBlockchainUserData() {
    ...
    // Load Info on FXCore
    let purseBalanceOnFXCore = await this.loadPurseBalanceOnFXCore()
    this.setState({ purseBalanceOnFXCore: purseBalanceOnFXCore.toString() })

    let earnedAmountOnFXCore = await this.loadEarnedAmountOnFXCore()
    this.setState({ earnedAmountOnFXCore: earnedAmountOnFXCore.toString() })
  }
  // ***************************Farm Info***********************************************************************
  ...
  // ***************************f(x)Core ***********************************************************************
  async loadPurseTotalSupplyOnFXCore() {
    let purseTotalSupplyOnFXCore = await this.state.tokenOnFXCore.methods.totalSupply().call()
    return purseTotalSupplyOnFXCore
  }

  async loadPurseBalanceOnFXCore() {
    let purseBalanceOnFXCore = await this.state.tokenOnFXCore.methods.balanceOf(this.state.account).call()
    return purseBalanceOnFXCore
  }

  async loadEarnedAmountOnFXCore() {
    let earnedAmountOnFXCore = await this.state.masterChef.methods.pendingReward(2, this.state.account).call() // FX-PURSE LP
    return earnedAmountOnFXCore
  }

  async loadMasterChef() {
    let totalAllocPoint = await this.state.masterChef.methods.totalAllocPoint().call()
    let rewardPerBlock = await this.state.masterChef.methods.rewardPerBlock().call()
    let poolInfo = await this.state.masterChef.methods.poolInfo(2).call()
    let fxRewardPerWeek = bigInt(rewardPerBlock * poolInfo[1] * 15000 * 7 / totalAllocPoint) // Reward per Week
    return fxRewardPerWeek
  }
  ...
  async loadTVLAPR() {
    ...
    let fxswapApy = fxswapJson["AllData"]["apyDaily"][2]
    this.setState({ fxswapTvl })
    this.setState({ fxswapApr })
    this.setState({ fxswapApy })
  }
  ...
  checkRetroactiveRewardsAmount = async (address) => {
    let newAddress = window.web3Bsc.utils.toChecksumAddress(address)
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      fxswapApr: '0',
      fxswapApy: '0',
      purseTotalSupplyOnFXCore: '0',
      fxRewardPerWeek: '0',
      purseBalanceOnFXCore: '0',
      earnedAmountOnFXCore: '0',
    }
  }
  ...
  render() {
    ...
    />
    fxswapContent = <FXSwap
      tvl={this.state.fxswapTvl}
      apr={this.state.fxswapApr}
      apy={this.state.fxswapApy}
      farmLoading={this.state.farmLoading}
      aprloading={this.state.aprloading}
      purseTotalSupplyOnFXCore={this.state.purseTotalSupplyOnFXCore}
      fxRewardPerWeek={this.state.fxRewardPerWeek}
              ...
              <main role="main" className="col-lg-12 ml-auto mr-auto" style={{ maxWidth: '1000px' }}>
                <div className="content mr-auto ml-auto" id="content">
                  <Switch>
                    ...
                    <Route path="/lpfarm/fxswap" exact > {fxswapContent} </Route>
