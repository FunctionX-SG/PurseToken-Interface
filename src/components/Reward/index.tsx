...
export default function Rewards() {
  ...
  const [retroactiveRewardsAmount, setRetroactiveRewardsAmount] =
    useState<BigNumber>(BigNumber.from("0"));
  const [retroactiveRewardsIsClaim, setRetroactiveRewardsIsClaim] =
    useState<Boolean>(false);
  const [retroactiveRewardsStartTime, setRetroactiveRewardsStartTime] =
    useState<number>(0);
  const [retroactiveRewardsEndTime, setRetroactiveRewardsEndTime] =
    useState<number>(0);
  ...
  useEffect(() => {
    async function getRewardData() {
      const _retroactiveRewardsStartTime =
        await retroactiveRewards.rewardStartTime();
      const _retroactiveRewardsEndTime =
        await retroactiveRewards.rewardEndTime();
      setRetroactiveRewardsStartTime(_retroactiveRewardsStartTime);
      setRetroactiveRewardsEndTime(_retroactiveRewardsEndTime);
      if (isActive === true) {
        const _retroactiveRewardsIsClaim = await retroactiveRewards.isClaim(
          account
        );
        const _retroactiveRewardsAmount =
          checkRetroactiveRewardsAmount(account);
  ...
  const checkRetroactiveRewardsAmount = (address: string | undefined) => {
    ...
    if (UserAmount[newAddress as keyof typeof UserAmount] !== undefined) {
      retroactiveRewardsAmount =
        UserAmount[newAddress as keyof typeof UserAmount]["Amount"];
    } else {
  ...
  const claimRetroactiveRewardsAmount = async () => {
    if (
      parseFloat((Date.now() / 1000).toFixed(0)) > retroactiveRewardsEndTime
    ) {
      showToast("Claim Ended", "failure");
    } else if (
      parseFloat((Date.now() / 1000).toFixed(0)) < retroactiveRewardsStartTime
    ) {
        ...
        >
          <div className="card-body">
            <span>
              ...
              <div className="textWhiteSmall ml-3 mb-2">
                <b>Remarks:</b>
              </div>
              <ul>
                <li className="textWhiteSmaller">
                  838 Million PURSE will be given out to Early Stakers as
                  Retroactive Rewards
                </li>
                <li className="textWhiteSmaller">
                  Make sure your wallet is connected to BSC network.
                </li>
                <li className="textWhiteSmaller">
                  Make sure you have sufficient BNB to pay for the transaction
                  fees.
                </li>
                <li className="textWhiteSmaller">
                  Click on Claim and confirm the transaction to claim your PURSE
                  tokens.
                </li>
                <li className="textWhiteSmaller">
                  Note that you cannot claim after the End Date.
                </li>
              </ul>
            </span>
          </div>
              ...
              <div className="card-body">
                <div>
                  <div>
                    ...
                    <div className="textWhiteSmall mb-2">
                      <b>{account}</b>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="textWhiteSmall mb-1">
                      {isLoading ? (
                        ...
                        <b>
                          {parseFloat(
                            formatUnits(retroactiveRewardsAmount, "ether")
                  ...
                  {isLoading ? (
                    ...
                    <div>
                      {parseFloat((Date.now() / 1000).toFixed(0)) >
                        retroactiveRewardsEndTime ||
                      parseFloat((Date.now() / 1000).toFixed(0)) <
                        retroactiveRewardsStartTime ? (
                        <div>
                          {parseFloat((Date.now() / 1000).toFixed(0)) <
                          retroactiveRewardsStartTime ? (
                            <div className="center mt-2 mb-4">
