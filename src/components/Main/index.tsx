...
export default function Main(props: any) {
    let cumulateTransfer = props.cumulateTransfer
    let cumulateBurn = props.cumulateBurn
    let newCumulateTransfer: {Sum: number; Date: string}[] = []
    let newCumulateBurn: {Sum: number; Date: string}[] = []
    cumulateTransfer.forEach((item:{Sum: string; Date: string}) => newCumulateTransfer.push({"Sum": parseFloat(item.Sum), "Date": item.Date}));
    cumulateBurn.forEach((item:{Sum: string; Date: string}) => newCumulateBurn.push({"Sum": parseFloat(item.Sum), "Date": item.Date}));
        ...
        <div className="card mb-4 cardbody">
          <div className="card-body center">
            <table className="textWhiteSmall">
              ...
              <tbody>
                <tr>
                  <td>${(parseFloat(formatUnits(props.purseTokenTotalSupply, 'ether')) * props.PURSEPrice).toLocaleString('en-US', { maximumFractionDigits: 0 })}</td>
                  <td>{parseFloat(formatUnits(props.purseTokenTotalSupply, 'ether')).toLocaleString('en-US', { maximumFractionDigits: 0 })}</td>
                  <td>${parseFloat(props.PURSEPrice).toLocaleString('en-US', { maximumFractionDigits: 6 })}</td>
                </tr>
              </tbody>
              ...
              <tbody>
                <tr>
                  <td>{parseFloat(formatUnits(props.totalBurnAmount, 'ether')).toLocaleString('en-US', { maximumFractionDigits: 0 })} / $ {(parseFloat(formatUnits(props.totalBurnAmount, 'ether')) * props.PURSEPrice).toLocaleString('en-US', { maximumFractionDigits: 0 })}</td>
                  <td>{parseFloat(formatUnits(props.totalTransferAmount, 'ether')).toLocaleString('en-US', { maximumFractionDigits: 0 })} / $ {(parseFloat(formatUnits(props.totalTransferAmount, 'ether')) * props.PURSEPrice).toLocaleString('en-US', { maximumFractionDigits: 0 })}</td>
                  <td>{parseFloat(formatUnits(props.totalTransferAmount, 'ether')).toLocaleString('en-US', { maximumFractionDigits: 0 })} / $ {(parseFloat(formatUnits(props.totalTransferAmount, 'ether')) * props.PURSEPrice).toLocaleString('en-US', { maximumFractionDigits: 0 })}</td>
                </tr>
              </tbody>
              <thead>
              ...
              <tbody>
                <tr>
                  <td>{parseFloat(formatUnits(props.sum30BurnAmount, 'ether')).toLocaleString('en-US', { maximumFractionDigits: 0 })} / $ {(parseFloat(formatUnits(props.sum30BurnAmount, 'ether')) * props.PURSEPrice).toLocaleString('en-US', { maximumFractionDigits: 0 })}</td>
                  <td>{parseFloat(formatUnits(props.sum30TransferAmount, 'ether')).toLocaleString('en-US', { maximumFractionDigits: 0 })} / $ {(parseFloat(formatUnits(props.sum30TransferAmount, 'ether')) * props.PURSEPrice).toLocaleString('en-US', { maximumFractionDigits: 0 })}</td>
                  <td>{parseFloat(formatUnits(props.sum30TransferAmount, 'ether')).toLocaleString('en-US', { maximumFractionDigits: 0 })} / $ {(parseFloat(formatUnits(props.sum30TransferAmount, 'ether')) * props.PURSEPrice).toLocaleString('en-US', { maximumFractionDigits: 0 })}</td>
                </tr>
              </tbody>
            </table>
