...
export default function Deposit(props: any) {
    ...
    const deposit = async (amount:BigNumber) => {
      if (isActive) {
        ...
        try{
          const tx:any = await callContract(signer,restakingFarm,"deposit",lpTokenAddress,amount)
          onChangeHandler('')
          if (tx?.hash){
    ...
    const withdraw = async  (amount:BigNumber) => {
      if (isActive) {
        const restakingFarm = new ethers.Contract(Constants.RESTAKING_FARM_ADDRESS,RestakingFarm.abi)
        try{
          const tx:any = await callContract(signer,restakingFarm,"withdraw",lpTokenAddress,amount)
          onChangeHandler('')
          if (tx?.hash){
            const link = `https://bscscan.com/tx/${tx.hash}`
            showToast("Transaction sent!","success",link)
    ...
    const approve = async () => {
      if (isActive) {
        try{
          const tx:any = await callContract(signer,lpTokenContract,"approve",Constants.RESTAKING_FARM_ADDRESS,"115792089237316195423570985008687907853269984665640564039457584007913129639935")
          if (tx?.hash){
            const link = `https://bscscan.com/tx/${tx.hash}`
            showToast("Transaction sent!","success",link)
            await tx.wait()
          ...
          <div className="card mb-3 cardbody" style={{ fontFamily: 'Verdana', color: 'silver'}}>
            <div className="card-body">
              ...
              <table className="table table-borderless text-center" style={{ color: 'silver', fontSize:'15px' }}>
                ...
                <tbody>
                  <tr>
                    <td>{parseFloat(formatBigNumber(lpStaked?.amount, 'ether')).toLocaleString('en-US', { maximumFractionDigits: 5 })}</td>
                    <td>{parseFloat(formatBigNumber(purseEarned,'ether')).toLocaleString('en-US', { maximumFractionDigits: 3 })}</td>
                  </tr>
                </tbody>
              </table>
